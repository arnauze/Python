...
class Game(object):
	...
	def start(self, difficulty=difficulty):
		...
		print()
		user.informations(self.gold, self.special)
		...
		while i == 0:
			...
			while i == 0:
				...
				if choice == "3":
					print("You're using your special attack!")
					input()
					if user.classe == "Mage":
						print("You deal two times more damages")
						input()
						boss.takes_damages(user, "double")
						self.special = 0
						if boss.life <= 0:
							print("You won the fight and earned 10 gold!")
							input();
							...
							break
						print("Ennemy life: {}".format(boss.life))
					if user.classe == "Archer":
						print("You have two times more chances to dodge the next attack")
						input()
					if user.classe == "Guerrier":
						print("You attack and if you get touched you will receive twice less damages.")
						boss.takes_damages(user)
						self.special = 0
						print("You dealt {} damages!".format(user.damages))
						input()
				print("\nNow the ennemy is going to attack.")
				input()
				if user.classe == "Archer" and self.special == 5:
					n = random.randint(1, boss.hit_ratio * 2)
					self.special = 0
				else:
				...
				if n == 1:
					...
					input()
					if choice != "2":	
						if user.classe == "Guerrier" and choice == "3":
							print("You lost {} hp\n".format(boss.damages / 2))
							user.takes_damages(boss, "shield")
							self.special = 0
						else:
							print("You lost {} hp\n".format(boss.damages))
